...
declare
   ...
   -- 사원번호, 사원명, 입사일 을 저장할 수 있는 Record 설정      (이번에는 참조해서)
...
begin                                              --레코드는 인덱스가 들어가지 않는다
  ...
  dbms_output.put_line('번호 : '|| jaehyun_score.num||'이름 : '||
  										...
  										jaehyun_score.oracle_score||', 자바 점수: '||
  										jaehyun_score.java_score||', 총점 : '|| 
     ...
     dbms_output.put_line('사번 : ' ||emp1.empno||', 사원명 : '||
     												emp1.ename||', 연봉 : '||emp1.sal||', 입사일 : '||
...
dbms_output.put_line(' 연봉차 : '||abs(emp1.sal - emp2.sal)
								||', 입사 개월 차 : '||months_between(emp2.input_date,emp1.input_date));     												
