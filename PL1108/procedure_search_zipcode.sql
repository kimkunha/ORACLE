-- 동이름을 입력받아 우편번호 테이블에 주소 검색 하는 procedure
--우편번호의 오름차순 정렬했을때 가장 먼저 조회되는
...
is
	...
	type rec_zip is record(
			-- 변수명이 테이블명, 컬럼명과 같다면 인식에 대한 Bug가  발생 할 수 있다.
			 
			--zipcode zipcode.zipcode%type, 
			zip char(7),
			--위에 선언된 변수가 테이블 명과 같다면 테이블을
			--참조하지 못하게 될수도 있다.
