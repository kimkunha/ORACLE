...
-- 직급은 사원, 주임, 대리, 과장, 차장, 부장, 이사, 상무, 전무, 대표이사 가 입력되었을때만 변경 한다.

-- out parameter에 설정하는 procedure 

create or replace procedure select_test_proc_cnt
			(i_job varchar2,result_msg out varchar2,
			 emp_cnt out number, emp_sal out number)
is
		temp_sal number := 0;
...
--          		    end loop;     
			if emp_cnt !=0 then
			   	 result_msg := i_job||'으로 조회된 결과가 있음.';
          	else
          		result_msg := i_job||'직급의 사원은은 존재하지 않습니다.';  
          		emp_sal := 0;      
          	end if;
